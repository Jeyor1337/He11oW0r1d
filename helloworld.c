...
unsigned char get_key() {
    return (unsigned char)op_sub(100, 10);
}
void get_blob(unsigned char* buf) {
    buf[0] = op_sum(17, 1);         
    buf[1] = op_sub(70, 7);         
    buf[2] = op_mul(9, 6);          
    buf[3] = op_sum(50, 4);         
...
void phase_init(struct proc_t* ctx) {
    ...
    ctx->rot_key = 0;
    ctx->chk = 0;
    for (unsigned long i = 0; i < 7; i = op_inc(i))
    {
        char* p_buf = (char*)op_sum((unsigned long)ctx->buf, i);
        const unsigned char* p_src = (const unsigned char*)op_sum((unsigned long)local_blob, i);
        *p_buf = *p_src ^ key;
    }
...
void phase_prng(struct proc_t* ctx) {
    ...
    for (unsigned long i = 7; i < 14; i = op_inc(i))
    {
        char* p_buf = (char*)op_sum((unsigned long)ctx->buf, i);
        const unsigned char* p_src = (const unsigned char*)op_sum((unsigned long)local_blob, i);
        *p_buf = *p_src ^ key;
...
DISPATCH:
    ...
    if (ctx.st == 2) goto P_TRANSFORM;
    if (ctx.st == 3) goto P_CHECKSUM;
    if (ctx.st == 4) goto P_VALIDATE;
    if (ctx.st == 5) goto P_INDIRECT;
    if (ctx.st == 6) goto P_LEN;
    if (ctx.st == 7) goto P_EMIT;
    if (ctx.st == 8) goto P_HALT;
    goto DONE;
P_INIT:
    phase_init(&ctx);
    goto DISPATCH;
